package com.mx.curso.unidad4.arbol.postorden;

/**
 * Clase que representa un nodo individual en el Árbol Binario.
 */
class Nodo {
    int valor;
    Nodo izquierda; // Puntero al subárbol izquierdo
    Nodo derecha;   // Puntero al subárbol derecho

    public Nodo(int item) {
        valor = item;
        izquierda = derecha = null;
    }
}

/**
 * Clase principal que gestiona la estructura del Árbol Binario
 * y contiene el método de recorrido Postorden.
 */
public class ArbolBinarioPostorden {
    Nodo raiz;

    // Constructor para inicializar un árbol vacío
    ArbolBinarioPostorden() {
        raiz = null;
    }

    /**
     * Función recursiva para el Recorrido Postorden: Izquierda -> Derecha -> Raíz.
     * Este recorrido es útil para la eliminación o borrado de nodos.
     *
     * @param nodo El nodo actual en el recorrido.
     */
    void recorrerPostorden(Nodo nodo) {
        // La condición de parada (caso base) es cuando el nodo es nulo
        if (nodo != null) {
            // 1. Recorrer el subárbol izquierdo (RECURSIVIDAD)
            recorrerPostorden(nodo.izquierda);

            // 2. Recorrer el subárbol derecho (RECURSIVIDAD)
            recorrerPostorden(nodo.derecha);

            // 3. Visitar el nodo actual (IMPRIMIR LA RAÍZ AL FINAL)
            System.out.print(nodo.valor + " ");
        }
    }

    // Método auxiliar para llamar al recorrido desde la raíz
    void recorrido() {
        recorrerPostorden(raiz);
    }

    /**
     * Método principal para construir el árbol y ejecutar el recorrido.
     */
    public static void main(String[] args) {
        ArbolBinarioPostorden arbol = new ArbolBinarioPostorden();

        // Construcción manual del árbol (la misma estructura de BST):
        //        10
        //       /  \
        //      5    15
        //     / \   / \
        //    2   7 12  18

        arbol.raiz = new Nodo(10);
        arbol.raiz.izquierda = new Nodo(5);
        arbol.raiz.derecha = new Nodo(15);
        arbol.raiz.izquierda.izquierda = new Nodo(2);
        arbol.raiz.izquierda.derecha = new Nodo(7);
        arbol.raiz.derecha.izquierda = new Nodo(12);
        arbol.raiz.derecha.derecha = new Nodo(18);

        System.out.println("Arbol construido. El recorrido Postorden es (Izq -> Der -> Raiz):");
        arbol.recorrido();
        System.out.println("\n");
        // Salida esperada: 2 7 5 12 18 15 10
    }
}